...
(function () {
  ...
  async function checkProductOwnershipAndToggleButton() {
    try {
      ...
      console.log('[DeleteModal] Product Owner ID:', product.profileId || product.userId || product.createdBy);
      console.log('[DeleteModal] Product Owner Username:', product.profileUsername || product.username || product.ownerUsername);
      ...
      if (profileResponse && profileResponse.ok) {
        try {
          ...
          console.log('[DeleteModal] Usuário encontrado via /auth/me, ID:', currentUserId, 'Username:', currentUsername);
        } catch (e) {
      ...
      if (!currentUserId) {
        ...
        if (profileResponse && profileResponse.ok) {
          try {
            ...
            currentUserId = currentUser.id || currentUser.profileId;
            currentUsername = currentUser.username;
            console.log('[DeleteModal] Usuário encontrado via /profile, ID:', currentUserId, 'Username:', currentUsername);
          } catch (e) {
      ...
      if (!currentUserId && !currentUsername) {
        ...
        try {
          ...
          currentUsername = decoded.sub || decoded.username || decoded.preferred_username || decoded.name;
          currentUserId = decoded.id || decoded.profileId;
          if (currentUserId || currentUsername) {
            console.log('[DeleteModal] Encontrado no JWT - ID:', currentUserId, 'Username:', currentUsername);
          }
        } catch (e) {
      ...
      const productOwnerId = product.profileId || product.userId || product.createdBy;
      const productOwnerUsername = product.profileUsername || product.username || product.ownerUsername;
      
      // Converter para string para comparação segura
      const currentUserIdStr = currentUserId ? String(currentUserId).trim().toLowerCase() : '';
      const productOwnerIdStr = productOwnerId ? String(productOwnerId).trim().toLowerCase() : '';
      const currentUsernameStr = currentUsername ? String(currentUsername).trim().toLowerCase() : '';
      const productOwnerUsernameStr = productOwnerUsername ? String(productOwnerUsername).trim().toLowerCase() : '';
      ...
      const isOwnerById = currentUserIdStr && productOwnerIdStr && currentUserIdStr === productOwnerIdStr;
      const isOwnerByUsername = currentUsernameStr && productOwnerUsernameStr && currentUsernameStr === productOwnerUsernameStr;
      ...
      if (deleteBtn) {
        if (isOwner) {
          ...
          console.log('[DeleteModal]  Botão visível e habilitado (você é o dono)');
        } else {
      ...
      console.error('[DeleteModal] ❌ Erro ao verificar propriedade do produto:', error);
      const deleteBtn = document.getElementById('deleteProductBtn');
      if (deleteBtn) {
        deleteBtn.disabled = true;
        deleteBtn.style.opacity = '0.5';
