...
document.addEventListener("DOMContentLoaded", () => {
  ...
  let eventsGrid = document.querySelector(".user-products-grid");
  if (!eventsGrid && postsGrid) {
    eventsGrid = document.createElement("div");
    eventsGrid.className = "user-products-grid";
    productsGrid.parentNode.insertBefore(eventsGrid, productsGrid.nextSibling);
  }
  ...
  if(eventsGrid) {
    initializeProductsGrid(eventsGrid);
  }
...
function activateEventsTab() {
  eventsState.active = true;
  if (!eventsState.container) return;
  eventsState.container.style.display = "grid";

  const hasRenderedProducts = Boolean(
    eventsState.container.querySelector(".user-product-card")
  );

  if (
    !eventsState.initialized ||
    (!hasRenderedProducts && !eventsState.loading)
  ) {
    eventsState.initialized = true;
    eventsState.page = 0;
    eventsState.finished = false;
...
async function loadNextEventsPage() {
  if (eventsState.loading || productsState.finished) return;
  if (!eventsState.container) return;
  ...
  try {
    ...
    if (eventsState.page === 0 && products.length === 0) {
      showProductsEmptyState("Nenhum produto cadastrado.");
      eventsState.finished = true;
      return;
    }
...
function setEventsLoadingIndicator(isLoading) {
  if (!eventsState.container) return;
  let indicator = eventsState.container.querySelector(".events-loading");

  if (isLoading) {
    if (!indicator) {
      indicator = document.createElement("div");
      indicator.className = "events-loading";
      indicator.style.gridColumn = "1 / -1";
      indicator.style.textAlign = "center";
      indicator.style.padding = "16px";
      indicator.style.color = "#592e83";
      eventsState.container.appendChild(indicator);
    }
    indicator.textContent = "Carregando...";
  } else if (indicator) {
    indicator.remove();
